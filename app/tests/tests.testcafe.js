...
import { openteamPage } from './openteam.page';
...
test('Test that a user can access the Open Teams page and the filter is accessible', async (testController) => {
  await navBar.gotoSigninPage(testController);
  await signinPage.signin(testController, userCred.username, userCred.password);
  await navBar.gotoOpenTeams(testController);
  await openteamPage.isDisplayed(testController);
  await openteamPage.clickFilter(testController);
});
