...
const AdminEditTeamWidget = (props) => {
   ...
   */
  const submit = (data) => {
    const { name, description, gitHubRepo, _id } = data;
      ...
      >
        ...
        >
          <Card.Body style={{ paddingLeft: '3rem', paddingRight: '3rem' }}>
            ...
            <h4>Team Members:</h4>
            <ListGroup>
              {memberNamesAndGitHub.map((n) => (
                <ListGroup.Item key={n}>{n}</ListGroup.Item>
              ))}
            </ListGroup>
...
const AdminEditTeamCon = withRouter(({ match }) => {
  // Get the documentID from the URL field. See imports/ui/layouts/App.jsx for the route containing :_id.
  const documentId = match.params._id;
  const team = useTracker(() => Teams.findDoc(documentId));
  const members = useTracker(() => _.map(TeamParticipants.find(
    { teamID: team._id },
).fetch(), (tp) => Participants.findDoc(tp.participantID)));
  return {
    team,
    members,
  };
